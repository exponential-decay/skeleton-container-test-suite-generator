...
class ContainerPart:
    ...
    def __eq__(self, other):
        ...
        return self.pos == other.pos and self.offset == other.offset \
...
class SkeletonContainerGenerator:
    ...
    def __init__(self, containersig, standardsig, debug):
        ...
        if not self.java:
            sys.stdout.write(
                "Not using Jython. Writing ZIP containers only.\n")
        else:
    ...
    def __del__(self):
        ...
        sys.stdout.write(
            ...
            .format(self.ole2written + self.zipwritten))
    ...
    def generateskeletonfiles(self):
        container_id_to_puid_map = \
            self.mapcontaineridstopuids(self.containertree)
        filenamedict = \
            self.createcontainerfilenamedict(container_id_to_puid_map)
    ...
    def createcontainerfilenamedict(self, container_id_to_puid_map):
        ...
        idfilenamedict = {}
        StandardSignatureFileHandler = \
            DroidStandardSigFileClass(self.standardsig)
        ...
        if len(self.invalidpuids) > 0:
            container_id_to_puid_map = \
                self.removeinvalidpuidsfromidpuidmapping(
                    container_id_to_puid_map)

        duplicate_list = \
            [x for x, y in collections.Counter(puid_list).items() if y > 1]

        # swap keys so we can access dict via puid value
        puid2idmapping = \
            dict((value, key)
                 for key, value in container_id_to_puid_map.iteritems())
        ...
        for d in duplicate_list:
            for id in container_id_to_puid_map:
                if container_id_to_puid_map[id] == d:
                    ...
                    idfilenamedict[fmtid] = fmt.replace('/', '-') \
                        + '-container-signature-id-' + str(fmtid) \
                        + '.' \
    ...
    def handlecontainersignaturefilepaths(
            self, innerfilename, containerfilename):
            ...
            if subseq is not None:
                ...
                for sequences in subseq:
                    ...
                    if rightfrag is not None:
                        rminoff = 0 if rightfrag.attrib['MinOffset'] \
                            is None else rightfrag.attrib['MinOffset']
        ...
        if subs is True:
            if len(parts) > 0:
                if len(parts) > 1:
                    ...
                    bio = self.__preprocessbofs__(
                        bio, parts, containerfilename)
                    ...
                    bio = self.__writebytestream__(
                        containerfilename, bio,
                        parts[0].offset, parts[0].minoff,
                        parts[0].maxoff, parts[0].seq)
    ...
    def __preprocessbofs__(self, bio, parts, containerfilename):
        ...
        if parts[0] == parts[1]:
            equiv = True
            sys.stderr.write(
                containerfilename + " has equivalent BOF sequences" + "\n")
            for p in parts:
                sys.stderr.write(
                    str(p.offset) + " " + str(p.pos) + " " + str(p.minoff)
                    + " " + str(p.maxoff) + " " + str(p.seq) + " \n")
            parts[0].maxoff = len(parts[0].seq)/2 + 1
        elif int(parts[1].minoff) > 0:
            sys.stderr.write(
                containerfilename
                + " BOF two offset greater than zero" + "\n")
            ...
            for p in parts:
                sys.stderr.write(
                    str(p.offset) + " " + str(p.pos) + " "
                    + str(p.minoff) + " " + str(p.maxoff)
                    + " " + str(p.seq) + " \n")

        for p in parts:
            bio = self.__writebytestream__(
                containerfilename, bio,
                p.offset, p.minoff, p.maxoff,
                p.seq, equiv)

        return bio

    def __writebytestream__(
            self, containerfilename, bio, offset,
            minoff, maxoff, seq, equiv=False):
        if seq != '':
            sig2map = signature2bytegenerator.Sig2ByteGenerator()
            if offset == 'BOFoffset':
                if equiv is False:
...
def main():
    ...
    parser.add_argument(
        '--con', help='DROID Container Signature File.',
        default=False, required=True)
    parser.add_argument(
        '--sig', help='DROID Standard Signature File.',
        default=False, required=True)
    parser.add_argument(
        '--debug',
