...
class SkeletonContainerGenerator:
        ...
        for topelements in iter(containertree):
            if topelements.tag == 'ContainerSignatures':
                ...
                for container in topelements:
                    ...
                    if containerid in filenamedict: #TODO: Bug filtering too many filenames/ids out, e.g. 1030, fmt/412
                        ...
                        for f in files:
                            ...
                            if path == None:
                                ...
                                cf = self.handlecontainersignaturefilepaths(None, containerfilename)
                            else:
        ...
        if subs == True:
            if len(parts) > 0:
                if len(parts) > 1:
                    ...
                    bio = self.__writebytestream__(containerfilename, bio, parts[0].offset, parts[0].minoff, parts[0].maxoff, parts[0].seq)
    ...
    def __preprocessbofs__(self, bio, parts, containerfilename):
        ...
        for p in parts:
            bio = self.__writebytestream__(containerfilename, bio, p.offset, p.minoff, p.maxoff, p.seq, equiv)

        return bio

    def __writebytestream__(self, containerfilename, bio, offset, minoff, maxoff, seq, equiv=False):
        if seq != '':
